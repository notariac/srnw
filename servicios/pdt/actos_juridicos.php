...
WHERE EXTRACT('YEAR' FROM kardex.fecfirmae)='{$_REQUEST['anio']}' AND CAST(kardex.escritura AS INT)>0
";
//echo $sql;
...
for($i=0;$i<count($query);$i++){
    $abreviatura=$query[$i]['abreviatura'];    
        ...
        for($j=0;$j<count($rowactoj);$j++){
                $actojuridico=zerofill($rowactoj[$j]['idacto_juridico'],2);
                $sqldetallecuota="SELECT 
                  ...
                  WHERE idkardex={$query[$i]['idkardex']} AND idmoneda<>1
                  ";
...
//                        $idmoneda=$ejecmontos[$k]['idmoneda'];
                        ...
                        $idmonedamediopago=moneda_equi($idmoneda);
