...
/*
		if ($path == '{default}') {
			$path = MOXMAN::getConfig()->get('filesystem.path');

			// No default path found then grab the first file system root path
			if (!$path) {
				$fileSystems = MOXMAN::getFileSystemManager()->getFileSystems();
				$path = $fileSystems[0]->getRootPath();
			}
		}
*/
