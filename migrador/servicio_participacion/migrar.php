...
<?php
set_time_limit(0);	
$SqlM = "SELECT a.idtipoparticipacion, a.idservicio, 1, now() 
        FROM tipoparticipacion_servicio as a inner join servicios as b on a.idservicio = b.idservicio
        ORDER BY a.idtipoparticipacion ASC";
...
while($rowM = $ConnM->FetchArray($ConsultaM)){
    ...
    $Id         = $rowM[0];
    $IdServicio = $rowM[1];				
    $Sql = "SELECT * FROM servicio_participacion WHERE idparticipacion=".$Id." AND idservicio=".$IdServicio;
    $Consulta = $Conn->Query($Sql);
    $Numero_Rows = $Conn->NroRegistros($Consulta);		
    if ($Numero_Rows==0){
        $Sql="INSERT INTO servicio_participacion ";
        $Sql=$Sql."(idparticipacion, idservicio) ";
        $Sql=$Sql."VALUES(".$Id.", ".$IdServicio.");";
        $Consulta = $Conn->Query($Sql);
        if (!$Consulta) {die($Sql);}
    }		
