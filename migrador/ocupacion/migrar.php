...
<?php
set_time_limit(0);	
$SqlM = "SELECT idocupacion, descripcion, estado, 1, now() FROM ocupacion";
...
while($rowM = $ConnM->FetchArray($ConsultaM)){
    ...
    $IdUsuario	= $rowM[3];
    $FecRegistro	= $rowM[4];				
    $Sql = "SELECT * FROM ocupacion WHERE idocupacion='$Id'";
    $Consulta = $Conn->Query($Sql);
    $Numero_Rows = $Conn->NroRegistros($Consulta);		
    if ($Numero_Rows==0){
        $Sql="INSERT INTO ocupacion (idocupacion, descripcion, estado, idusuario, fechareg) VALUES(".$Id.", '".$Descripcion."', ".$Estado.", ".$IdUsuario.", '".substr($FecRegistro,0,10)."');";
        $Consulta = $Conn->Query($Sql);
        if (!$Consulta) {
            echo $Sql;
