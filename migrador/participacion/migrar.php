...
<?php
set_time_limit(0);
$SqlM = "SELECT idtipoparticipacion, descripcion, estado, 1, now() FROM tipoparticipacion ORDER BY idtipoparticipacion ASC";
...
while($rowM = $ConnM->FetchArray($ConsultaM)){
    ...
    $IdUsuario	= $rowM[3];
    $FecRegistro	= $rowM[4];				
    $Sql = "SELECT * FROM participacion WHERE idparticipacion='$Id'";
    $Consulta = $Conn->Query($Sql);
    $Numero_Rows = $Conn->NroRegistros($Consulta);		
    if ($Numero_Rows==0){
        $Sql="INSERT INTO participacion ";
        $Sql=$Sql."(idparticipacion, descripcion, estado, idusuario, fechareg) ";
