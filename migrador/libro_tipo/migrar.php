...
<?php
set_time_limit(0);	
$SqlM = "SELECT idtipolibro, descripcion, estado, 1, now() FROM tipolibro";
...
while($rowM = $ConnM->FetchArray($ConsultaM)){
    ...
    $IdUsuario      = $rowM[3];
    $FecRegistro    = $rowM[4];				
    $Sql = "SELECT * FROM libro_tipo WHERE idlibro_tipo='$Id'";
    $Consulta = $Conn->Query($Sql);
    $Numero_Rows = $Conn->NroRegistros($Consulta);		
    if ($Numero_Rows==0){
        $Sql="INSERT INTO libro_tipo (idlibro_tipo, descripcion, estado, idusuario, fechareg) ";
        ...
        $Sql="UPDATE libro_tipo SET ";
        $Sql=$Sql." descripcion = '".$Descripcion."', estado = ".$Estado.", idusuario = ".$IdUsuario.", fechareg = '".substr($FecRegistro,0,10)."' ";
        $Sql=$Sql." WHERE idlibro_tipo=".$Id;
        $Consulta = $Conn->Query($Sql);
        if (!$Consulta) {
            echo $Sql;
            die("Error en Consulta SQL: ".pg_last_error());
