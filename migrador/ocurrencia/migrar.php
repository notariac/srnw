...
<?php
set_time_limit(0);	
$SqlM = "SELECT idocurrencia, descripcion, estado, 1, now() FROM ocurrencia";
...
while($rowM = $ConnM->FetchArray($ConsultaM)){
    ...
    $IdUsuario      = $rowM[3];
    $FecRegistro    = $rowM[4];				
    $Sql = "SELECT * FROM ocurrencia WHERE idocurrencia='$Id'";
    $Consulta = $Conn->Query($Sql);
    $Numero_Rows = $Conn->NroRegistros($Consulta);		
    if ($Numero_Rows==0){
        $Sql="INSERT INTO ocurrencia (idocurrencia, descripcion, estado, idusuario, fechareg) VALUES(".$Id.", '".$Descripcion."', ".$Estado.", ".$IdUsuario.", '".substr($FecRegistro,0,10)."');";
        $Consulta = $Conn->Query($Sql);
        if (!$Consulta) {
            echo $Sql;
