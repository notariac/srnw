...
if(!session_id()){ session_start(); }
    ...
    $n = ") ILIKE '%$Valor%' )";
    $pos = strpos($Sql, $n);
    $Sql = substr_replace($Sql," AND kardex.idnotaria='".$_SESSION["notaria"]."'",$pos+strlen($n),0);    
...
?>
        ...
        <?php
            if($i==6){
                if(strpos(strtoupper($row[$i-1]),"!")){
                    $Nombres = explode("!",strtoupper($row[$i-1]));
                    echo $Nombres[1]." ".$Nombres[0];
                }else{
                    echo strtoupper($row[$i-1]);
                }
            }else{
                echo strtoupper($row[$i-1]);
            }
...
<?php			
			for ($ii=1; $ii<=$FormatoGrilla[9]['NB']; $ii++){
				if (($row[$FormatoGrilla[9]['BtnCI'.$ii]-1]==$FormatoGrilla[9]['BtnCV'.$ii] && $row[4]==$_SESSION['id_user']) || ($FormatoGrilla[9]['BtnId'.$ii]=='BtnVer')){
?>
			<img width="15" id="<?php echo $row[0]?>" src="<?php echo $_SESSION["urlDir"]?>imagenes/iconos/<?php echo $FormatoGrilla[9]['BtnI'.$ii];?>" <?php echo $FormatoGrilla[9]['BtnF'.$ii];?> title="<?php echo $FormatoGrilla[9]['Btn'.$ii];?>" <?php echo $FormatoGrilla[9]['BtnF'.$ii];?> style="cursor:pointer;"/>
<?php
                                }else{
?>
