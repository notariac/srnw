...
class PDF extends FPDF{
        function Header(){
            ...
            $this->CabeceraTabla($Fecha,$Fechaf);
        }		
        function CabeceraTabla($Fecha,$Fechaf){
            global $Cabecera, $CabeceraW;			
            $this->SetFillColor(255, 255, 255);
            $this->SetTextColor(0);
            $this->SetLineWidth(.3);
            $this->SetFont('Arial', 'B', 7);			
            $this->Cell(170, 5, "Consulta de la Fecha : ".$Fecha." hasta ".$Fechaf, 0, 1, 'L', 1);
        ...
        function Detalle(){
            global $Conn, $CabeceraW, $Fecha, $IdComprobante, $TipoPago, $Fechaf;			
            $SQL = "SELECT facturacion.facturacion_fecha, 
                    comprobante.abreviatura, 
                    facturacion.comprobante_serie, 
                    facturacion.comprobante_numero, 
                    facturacion.dni_ruc, 
                    facturacion.nombres, 
                    forma_pago.descripcion, 
                    CASE WHEN facturacion.estado=0 THEN 'PENDIENTE' 
                         WHEN facturacion.estado=1 THEN 'CANCELADO' 
                         WHEN facturacion.estado=2 THEN 'ANULADO' ELSE '' END, 
                    CASE WHEN facturacion.igv_afecto=0 then 'NO AFECTO' 
                         WHEN facturacion.igv_afecto=1 THEN 'AFECTO' ELSE '' END, facturacion.total 
                    FROM facturacion INNER JOIN comprobante ON (facturacion.idcomprobante = comprobante.idcomprobante) 
                        INNER JOIN forma_pago ON (facturacion.idforma_pago = forma_pago.idforma_pago) 
                    WHERE facturacion.idforma_pago not in(5,7) 
                        and facturacion.facturacion_fecha between '$Fecha' and '$Fechaf'
                        AND facturacion.idnotaria='".$_SESSION['notaria']."' 
		    ";
            if ($IdComprobante!=''){$SQL = $SQL." AND facturacion.idcomprobante = '$IdComprobante' ";}
            if ($TipoPago!=''){
                $SQL = $SQL." AND facturacion.idforma_pago = '$TipoPago' ";                
            }	
            $SQL .= " ORDER BY facturacion.comprobante_numero, comprobante.abreviatura";			
