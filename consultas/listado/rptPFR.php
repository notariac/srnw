...
class PDF extends FPDF{
	...
	$Cab = array('NRO.', 'OTORGANTE(S)', 'APODERADO(S)', 'MOTIVO(S)', 'FECHA');
	$w = array(10, 65, 65, 40, 20);
	// Carga de datos
	$SQL = 	"SELECT kardex.idkardex, kardex.correlativo, kardex.escritura, kardex.motivo, kardex.escritura_fecha FROM kardex INNER JOIN servicio ON (servicio.idservicio = kardex.idservicio) WHERE substr(kardex.correlativo, 1, 1) = 'P' AND kardex.escritura_fecha BETWEEN '$Desde' AND '$Hasta' AND kardex.idnotaria='".$_SESSION['notaria']."' ORDER BY kardex.escritura ASC";
