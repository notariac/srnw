...
class PDF extends FPDF{
	function LoadData($data){
		...
		while($row	= $Conn->FetchArray($data)){
                    ...
                    $this->MultiCell($w[4], $Lin, $Fecha, 1, 'C', false);
                        $posX = $posX + $w[4];
                        $this->SetXY($posX, $posY);
                    $this->MultiCell($w[5], $Lin, $row[5], 1, 'C', false);
		}
	}
	// Una tabla más completa
	function Titulo(){
            $this->SetFont('Arial', '', 14);
            $this->Cell(0, 6, 'INDICE DE TRANSFERENCIA VEHICULAR', 0, 1, 'C');
            $this->SetFont('Arial', '', 10);
            $this->Cell(0, 5, '(ORDEN ALFABETICO)', 0, 1, 'C');
            $this->Ln();
	}
}
	$pdf = new PDF();
	...
	$Cab = array('NRO.', 'VENDEDOR', 'COMPRADOR', 'PLACA', 'FECHA', 'FOLIO');
	$w = array(18, 60, 60, 20, 20, 20);
	// Carga de datos
	$SQL = 	"SELECT kardex.idkardex, kardex.correlativo, kardex.escritura, kardex.placa, kardex.escritura_fecha, kardex.fojafin FROM kardex INNER JOIN servicio ON (servicio.idservicio = kardex.idservicio) WHERE substr(kardex.correlativo, 1, 1) = 'V' AND kardex.escritura_fecha BETWEEN '$Desde' AND '$Hasta' AND kardex.idnotaria='".$_SESSION['notaria']."' ORDER BY servicio.descripcion ASC";
