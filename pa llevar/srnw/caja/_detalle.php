...
if(!session_id()){ session_start(); }
    ...
    $Anio = $_POST['Anio'];
    $Id = $_POST['Id'];	
    $Sql = "SELECT COUNT(*) FROM atencion WHERE correlativo = '$Id' AND idnotaria='".$_SESSION['notaria']."' AND estado!='2' ";	
    $Consulta = $Conn->Query($Sql);	
    $row = $Conn->FetchArray($Consulta);	
    $SqlX = "SELECT idatencion FROM atencion WHERE correlativo = '$Id' AND idnotaria='".$_SESSION['notaria']."' AND estado!='2' ";	
    $ConsultaX = $Conn->Query($SqlX);	
    $rowX = $Conn->FetchArray($ConsultaX);	
    $SqlTF = "SELECT * FROM facturacion WHERE idatencion = '".$rowX[0]."' ";
...
<?php
			...
			if ($row[0]>0){
				$NumRegsA = 1;
				$SQL2 = "SELECT atencion_detalle.anio, atencion.idatencion, atencion_detalle.item, atencion_detalle.idservicio, servicio.descripcion, atencion_detalle.correlativo, atencion_detalle.cantidad, atencion_detalle.monto, (atencion_detalle.cantidad * atencion_detalle.monto) FROM atencion_detalle INNER JOIN servicio ON (atencion_detalle.idservicio = servicio.idservicio) INNER JOIN atencion ON (atencion.idatencion = atencion_detalle.idatencion) WHERE atencion.correlativo='$Id' AND atencion_detalle.idnotaria='".$_SESSION['notaria']."' ORDER BY atencion_detalle.item ASC";				
