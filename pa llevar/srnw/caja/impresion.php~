...
class PDF extends FPDF{
  ...
  function LoadData($data,$Id)
  {
	...
	if($t>45){ $add="...";}
      ...
      $this->MultiCellp(95,$h,strtoupper(trim($row2[4])),$border,'J',false);
      //$this->Cell(95,$h,strtoupper($row2[4]),$border,'J',false);
      $this->Cell(25,$h,$row2[5],$border,0,'C',false);  
      $this->Cell(20,$h,number_format($row2[7],2),$border,0,'R',false); 
      $this->Cell(25,$h,number_format($row2[8],2),$border,1,'R',false);  
    }
    $h=5;
    $this->Ln(15);
    $this->SetFont('Arial', '', 10);
    //$this->Cell(0,$h,strtoupper("Observacion: ".$row['observaciones']),$border,1,'L',false);
    
    //$row['observaciones'] = substr($row['observaciones'],0,160).".";
    $row['observaciones'] = utf8_decode($row['observaciones']);        

    $h = $h -1;
    $this->MultiCell(120,$h,strtoupper("Observacion: ".$row['observaciones']." "),$border,'J',false);
    ...
    $this->Cell(25,$h,number_format($t,2),$border,1,'R',false);
    ...
    $this->Cell(25,$h,number_format($row[16],2),$border,1,'R',false);   
  }
