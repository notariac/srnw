...
if(!session_id()){ session_start(); }
   ...
   // if($fv[1]==1){
    $output.= "{\\qj\\fs28 Que, el original de la presente Carta Notarial ha sido diligenciada el ".utf8_decode(día)." ".$mifecha[2]." de ".ucwords(strtolower($Meses[(int)$mifecha[1]]))." del ".$mifecha[0].", }";    
    ...
    //else{
        //$output.= "{\\ QUE, SIENDO LAS ".$HoraH." CON ".$HoraM." DEL DIA ".trim(CantidadEnLetra((int)$mifecha[2]))." DE ".strtoupper($Meses[(int)$mifecha[1]])." DEL ".trim(CantidadEnLetra((int)$mifecha[0])).", }";
          $output.= "{\\qj\\fs28 en ".utf8_decode(dirección)." indicada en la misma, ".utf8_decode(strtolower($row[9]))." distrito de ".ucwords(strtolower(utf8_decode($Distrito))).", }";
    //}
    
    $output.= "{\\qj\\fs28 siendo recepcionada por el destinatario, quien ". utf8_decode(firmó) ." el presente duplicado. Doy ".utf8_decode(fé).".}";
    $output.="\\qj\\par";
    $output.= "{\\qj\\fs28 Ciudad de Tarapoto, ".$mifecha[2]." de ".ucwords(strtolower($Meses[(int)$mifecha[1]]))." del ".$mifecha[0]."3}";
    $output.= "}\\qj"; 
    echo $output;
?>