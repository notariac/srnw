                    ...
                    if ($rowCorr[0]==1)
                    {
                            ...
                            if ($_POST['0formD'.$i.'_idservicio']==118 || $_POST['0formD'.$i.'_idservicio']==245){
				                ...
				                if($temp_corr==''){
                                    ...
                                    //$ConsultaCoX = $Conn->Query("SELECT DISTINCT kardex_tipo_notaria.actual,kardex_tipo.abreviatura FROM servicio INNER JOIN atencion_detalle ON servicio.idservicio=atencion_detalle.idservicio INNER JOIN atencion ON atencion.idatencion = atencion_detalle.idatencion INNER JOIN kardex ON kardex.idatencion = atencion.idatencion INNER JOIN kardex_tipo ON kardex_tipo.idkardex_tipo = servicio.idkardex_tipo
                                    
                                    $ConsultaCoX = $Conn->Query("SELECT DISTINCT kardex_tipo_notaria.actual,kardex_tipo.abreviatura FROM servicio INNER JOIN kardex_tipo ON kardex_tipo.idkardex_tipo = servicio.idkardex_tipo INNER JOIN kardex_tipo_notaria ON kardex_tipo.idkardex_tipo = kardex_tipo_notaria.idkardex_tipo left outer JOIN atencion_detalle ON servicio.idservicio=atencion_detalle.idservicio left outer JOIN atencion ON atencion.idatencion = atencion_detalle.idatencion and atencion.anio >= '".$_POST['0formD'.$i.'_anio']."'  WHERE kardex_tipo_notaria.idnotaria='".$_SESSION['notaria']."' AND kardex_tipo_notaria.idkardex_tipo = '".$rowCorr[1]."' ");                                    

                                    $rowCoX = $Conn->FetchArray($ConsultaCoX);
				                    $p = $rowCoX[1];

                                    $numero = $mantem->generaCode($rowCoX[0]-1, $p, 6);
                                    //echo " prefijo = ".$p;
                                    $actualK = $numero;
                                    $SqlK = "INSERT INTO kardex(idatencion, fecha, correlativo, idservicio, idusuario, fechareg, anio, idnotaria) 
						                      VALUES ('$IdAtencion', '".$_POST['3form1_fecha']."', '".$numero."', '".$_POST['0formD'.$i.'_idservicio']."', '".$_POST['0form1_idusuario']."', '".date('Y-m-d')."', '".$_POST['0formD'.$i.'_anio']."', '".$_SESSION["notaria"]."')";
                                    //echo $SqlK." ".$actualK;
