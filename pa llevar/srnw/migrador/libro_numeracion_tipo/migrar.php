...
<?php
set_time_limit(0);	
$SqlM = "SELECT idnumeracion, descripcion, estado, 1, now() FROM tiponumeracion";
...
$Cont = 0;
    while($rowM = $ConnM->FetchArray($ConsultaM)){
		...
		$IdUsuario	= $rowM[3];
		$FecRegistro	= $rowM[4];				
		$Sql = "SELECT * FROM libro_numeracion_tipo WHERE idlibro_numeracion_tipo='$Id'";
		$Consulta = $Conn->Query($Sql);
		$Numero_Rows = $Conn->NroRegistros($Consulta);		
		if ($Numero_Rows==0){
                    $Sql="INSERT INTO libro_numeracion_tipo (idlibro_numeracion_tipo, descripcion, estado, idusuario, fechareg) VALUES('$Id', '$Descripcion', '$Estado', '$IdUsuario', '".substr($FecRegistro,0,10)."')";
                    $Consulta = $Conn->Query($Sql);
                    if (!$Consulta) {
                        echo $Sql;
                    ...
                    $Sql="UPDATE libro_numeracion_tipo SET descripcion = '$Descripcion', estado = '$Estado', idusuario = '$IdUsuario', fechareg = '".substr($FecRegistro,0,10)."' WHERE idlibro_numeracion_tipo='$Id'";
                    $Consulta = $Conn->Query($Sql);
                    if (!$Consulta) {
                            echo $Sql;
                            die("Error en Consulta SQL: ".pg_last_error());
