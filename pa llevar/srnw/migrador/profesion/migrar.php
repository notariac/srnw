...
<?php
set_time_limit(0);
$SqlM = "SELECT idprofesion, descripcion, estado, 1, now() FROM profesion";
...
while($rowM = $ConnM->FetchArray($ConsultaM)){
    ...
    $IdUsuario 	 = $rowM[3];
    $FecRegistro = $rowM[4];				
    $Sql = "SELECT * FROM profesion WHERE idprofesion='$Id'";
    $Consulta = $Conn->Query($Sql);
    $Numero_Rows = $Conn->NroRegistros($Consulta);		
    if ($Numero_Rows==0){
        $Sql="INSERT INTO profesion (idprofesion, descripcion, estado, idusuario, fechareg) VALUES('$Id', '$Descripcion', '$Estado', '$IdUsuario', '".substr($FecRegistro,0,10)."')";
        $Consulta = $Conn->Query($Sql);
        if (!$Consulta) {
            echo $Sql;
