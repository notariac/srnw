<?php 
    ...
    if($r['parte']=="")
    {
        ...
        {
            echo "<script>alert('Para registrar la parte, primero se tiene que tener elaborado el documento');window.close();</script>";
        }
    }
    else 
    {       
        $plantilla = stripSlash(html_entity_decode($r['parte']));       
        $plantilla = str_replace('"Times New Roman"',"'Times New Roman'",$plantilla);
	    $plantilla = utf8_decode($plantilla);
    }    
    if($plantilla=="")
...
function saveKardex()
{
    var idkardex = $("#idkardex").val(),
        ...
        $.post('save_parte.php',str,function(data){            
...
function myPrint()
{
   ...
   window.open('../editor/print_parte.php?idkardex='+kard,'width=600,height=300');
}
...
<form method="post" action="index.php" name="frm" id="frm">
    <div style="width:100%; padding:5px 0; background:green">
        ...
        <span style="width:300px; color:#FFFFFF; font-size:12px; margin: 0 10px 0 0">EDIDCION DIGITAL (Parte): <b><?php echo $r['correlativo']; ?></b></span>
